...
geometry_msgs::Vector3 floatmsg;
...
// TODO envoyer plusieurs valeurs d'odométrie (chaque roue)
// idee, si tableaux trop durs: balancer du Int64 avec 10000*TotalCodeurG + TotalCodeurD 
void messageCb(const std_msgs::Int16& toggle_msg){ // ]]]]]]]]]]]
   if (toggle_msg.data<1000 && stop==1)
   {
      distancecm=toggle_msg.data;
      avancer=1;
      time=0;
      stop=0;
   }
   else if (toggle_msg.data<2000 && stop==1)
   {
      distancecm=toggle_msg.data-1000;
      reculer=1;
      time=0;
      stop=0;
   }
   else if (toggle_msg.data<3000 && stop==1)
   {
      rotationdegres=toggle_msg.data-2000;
      TournerGauche=1;
      time=0;
      stop=0;
   }
   else if (toggle_msg.data<4000 && stop==1)
   {
      rotationdegres=toggle_msg.data-3000;
      TournerDroite=1;
      time=0;
      stop=0;   
   }
...
float diamrouecm=12.1;
float distancerouecm = 33; 
float CMG = 1;
...
void chatterBoy2()
{
	...
	{
		intmsg.data = 0; // distancecm + rotationdegres; 
...
if (( ((TotalCodeurG+TotalCodeurD)*diamrouecm*3.14)/(4*rapport_reducteur*tick_par_tour_codeuse) >= distancecm ) && (tourner==0))
{
	...
	//digitalWrite(IN4,HIGH);
     ...
     analogWrite(MotorD, cmdD);
     analogWrite(MotorG, cmdG);
     chatterBoy2(); /* TEST TOUURNAGAUCHE */
     distancecm=0;
     //TotalCodeurG=0;
     //TotalCodeurD=0;
     //TotalCodeurG=0;
...
// TODO: d'abord commencer par ajouter un subscriber qui reçoit les commandes
// SOUS FORME D'ACTIONS ELEMENTAIRE plannif/slam...SLAM!!
